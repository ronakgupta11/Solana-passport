...
import StampCard from '@/components/app/StampCard';
function Page() {
...
const collectedStampsNames = stamps?.map(stamp => stamp.name); // Extract names of collected stamps

// Filter the data to show only stamps you haven't collected
const collectedStamps = data.filter(item => collectedStampsNames.includes(item.name));

  return (
    <div className='flex flex-col space-y-4'>
        <div className='flex flex-col border-t mt-4 py-8 '>
            <p className='text-3xl md:text-5xl text-white font-bold '>Collected Stamps</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-8">
                {collectedStamps.map((d,k)=>{
                    return(
    <StampCard logo={"xyzz"} title={d.name} description={d.description}  key={k} />
                    )
                })}
